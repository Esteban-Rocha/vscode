...
class SuggestionDetails {
	...
	render(item: ICompletionItem, top: number): void {
		if (!item) {
...
export class SuggestWidget implements IContentWidget, IDelegate<ICompletionItem>, IDisposable {
	...
	private renderDetails(): void {
		if (this.state !== State.Details) {
			this.details.render(null, 0);
		} else {
			this.details.render(this.list.getFocusedElements()[0], this.list.elementTop(this.focusedItemIndex) - this.list.scrollTop);
		}
	}
